...
Le jeu de données à utiliser est le Online Retail II (UCI) , il contient les transactions e‑commerce d’un détaillant UK entre 01/12/2009 et le  09/12/2011(~1,07M lignes).
# Objectifs
...
- Politiques de remise/retours : quantifier effet sur marge et étention.
...
- **Scénarios (paramètres de simulation) À livrer**

    sliders marge %, rétention (r), taux d’actualisation (d), remise moyenne % (avec choix d’application globale ou par segment RFM), commutateur inclure retours, sélecteur cohorte cible (ou toutes). Pourquoi : quantifier Δ CLV / Δ CA / Δ rétention pour aider à décider.

- **Exports À livrer**

    CSV des données filtrées et des listes activables, PNG des graphiques (avec titre, date, filtres actifs). Pourquoi : traçabilité, partage, passage à l’action.
